...
onMounted(async () => {
  ...
  try {
    ...
    loading.value = false
    console.log('✅ تم تحميل الأسئلة:', all.value)
  } catch (error) {
...
<template>
  ...
  <main v-else-if="currentEntry" class="max-w-4xl mx-auto p-6">
    ...
    <header class="flex items-center justify-between mb-6">
      <div class="text-2xl font-bold" :class="s.state.ui?.projector ? 'text-3xl' : 'text-2xl'">
        {{ s.state.current?.difficulty }} نقطة
      </div>
      <div class="text-xl" :class="[
        timeLeft <= 10 ? 'text-red-500 animate-pulse' : 'text-gray-600', 
        s.state.ui?.projector ? 'text-2xl' : 'text-xl'
      ]">
        {{ Math.floor(timeLeft / 60) }}:{{ (timeLeft % 60).toString().padStart(2, '0') }}
      </div>
      <button @click="backToBoard" 
              class="rounded-lg border px-4 py-2" 
              :class="s.state.ui?.projector ? 'text-lg px-6 py-3' : ''">
        إلغاء
      </button>
    </header>
    ...
    <section class="rounded-xl border p-8 mb-8">
      <h1 class="text-3xl font-bold text-center mb-6" 
          :class="s.state.ui?.projector ? 'text-5xl' : 'text-3xl'">
        السؤال
      </h1>
      
      <div class="text-xl text-center mb-6" 
           :class="s.state.ui?.projector ? 'text-3xl' : 'text-xl'">
        {{ currentEntry.q }}
      </div>
      
      <!-- Media if available -->
      <div v-if="currentEntry.media?.length" class="space-y-4">
